...
import { findBestSummaryId, resolveContextNeeds, toPositiveInt } from "./assistantChat.util.mjs";
...
export const setWikiToDataPreviewCache = ({ userId, projectId, pageId, data }) => {
...
export const consumeWikiToDataPreviewCache = ({ key, userId, projectId, pageId }) => {
  cleanupWikiToDataPreviewCache();
...
export const classifyUserRequestWithModel = async ({ ai, model, message, history, locale }) => {
...
export const classifyActionIntentWithModel = async ({ ai, model, message, history, locale }) => {
  const compact = (value, max) => String(value || "").replace(/\s+/g, " ").trim().slice(0, max);
...
export const shouldRunClassifier = ({ fallbackIntent, fallbackDetailRequest }) => {
...
export const isTemporaryUpstreamError = (error) => {
  const statusCode = Number(error?.status) || Number(error?.error?.code) || 0;
  const statusText = String(error?.error?.status || "").toUpperCase();
  const message = String(error?.message || "");
  return (
...
export const detectWikiToDataIntent = (message, history = []) => {
...
export const detectExecutionConfirm = (message) => hasAnyTerm(message, WIKI_TO_DATA_CONFIRM_KEYWORDS);

export const hasPendingWikiToDataPreview = (history = []) => {
...
export const extractPageIdFromConversation = (message, history) => {
...
export const resolveActionPageId = async ({
  projectId,
  message,
  history,
...
export const extractTableNameHint = (message) => {
...
export const getPageDetail = async (projectId, pageId) => {
...
export const getProjectContextByIntent = async ({ projectId, intent }) => {
...
export const runWikiToDataAction = async ({
  userId,
  projectId,
  pageId,
  tableNameInput,

        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-for="row in rows" v-else :key="row.purchased_license_id">
              ...
              <td>
                <span :class="['status-pill', String(row.status || '').toUpperCase() === 'ACTIVE' ? 'ok' : 'muted']">
...
const fetchUsage = async () => {
  ...
  try {
    ...
    if (usageType !== "WORKSPACE_MEMBER_WORKSPACE_PURCHASERS") {
      loadError.value = "워크스페이스 멤버 슬롯 구매 워크스페이스 데이터가 아닙니다.";
      rows.value = [];
      return;
    }
